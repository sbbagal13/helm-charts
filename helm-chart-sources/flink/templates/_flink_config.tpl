...
{{- define "flink.configuration" -}}
    ...
    s3.access-key: {{ .Values.flink.s3Fs.accessKey }}
    s3.secret-key: {{ .Values.flink.s3Fs.secretKey }}
    s3.endpoint: {{ .Values.flink.s3Fs.endpoint }}
    s3.path.style.access: {{ .Values.flink.s3Fs.pathStyleAccess }}
    plugin.classloader.parent-first-patterns.additional: org.apache.kafka
    classloader.resolve-order: parent-first
    {{- if .Values.jobmanager.persistent.enabled }}
    jobmanager.archive.fs.dir: file:///{{ .Values.jobmanager.persistent.mountPath }} /completed-jobs
    web.upload.dir: /{{ .Values.jobmanager.persistent.mountPath }}
    {{- end }}
{{- end -}}
